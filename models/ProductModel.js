...
export const processBarcodeFromImage = async (base64Image) => {
  const startTime = Date.now();  // Start timer
    try {
      ...
      const endTime = Date.now();  // Start timer
      console.log(`Time taken to fetch GoogleVisionAPI response: ${endTime - startTime} milliseconds`);
      if (!response.ok) {
  ...
  export const fetchOpenAIResponse = async (imageURL) => {
    ...
    try {
      ...
      const endTime = Date.now();  // Start timer
      console.log(`Time taken to fetch OpenAI API response: ${endTime - startTime} milliseconds`);
  ...
  export const fetchOpenFoodFactsData = async (barcode) => {
    ...
    try {
      ...
      const data = await response.json();
      const endTime = Date.now();  // Start timer
      console.log(`Time taken to fetch OpenFoodFactAPI response: ${endTime - startTime} milliseconds`);
      if (data.status === 1) {
